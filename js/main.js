let app = new Vue({
    ...
    methods: {
        ...
        nextQuestion(answer) {
            ...
            // eval выполняет код из переданной строки
            eval(answer);
        },
        endGame() {
            this.totalGames++;
            ...
            if (this.score.zerg > this.score.protoss &&
                this.score.zerg > this.score.terran && this.score.primal < 8 &&
    ...
    // методы которые вызываются в момент изменения их возвращаемых значений
    // вычисляемые свойства хранятся в кэше
    computed: {
        totalScore() {
            let score = 0;
